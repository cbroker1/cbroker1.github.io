...
export function retrieve(
  ...
  // A missing subject is a hard decline: the model never sees a question about
  // something the site doesn't cover. Other unknown terms only soften the score.
  const unknownRatio = terms.length > 0 ? unknownTerms.length / terms.length : 0;
  const confidence = missingSubjects.length ? 0 : rawConfidence * (1 - unknownRatio * 0.5);
