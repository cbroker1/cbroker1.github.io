...
description: "XGBoost classifier with Optuna hyperparameter optimization for predictive yard operations and equipment movement."
date: 2026-07-04
tags:
  - Python
  - XGBoost
...
featured: true
status: "complete"
sourceNote: "Client project — details generalized for public sharing."
---

## Overview

For a logistics client, I built a predictive classification system to optimize yard operations — specifically, to predict which equipment and trailers needed movement and which assignments would generate the highest operational value. The system used XGBoost classifiers tuned with Optuna's Bayesian hyperparameter optimization, evaluated through decile-based gains analysis and model comparison. The approach converted a manual dispatching problem into a ranked priority system: instead of yard operators guessing which moves would matter most, the model produced a scored queue that focused attention on the highest-impact actions first.

This page is a generalized writeup. Client identifiers, proprietary data structures, and specific business metrics are omitted; the architecture, methods, evaluation approach, and technical decisions are real.

---

## The Problem

Distribution yards operate on tight margins. Every minute a trailer sits idle, every unnecessary move across the yard, every missed appointment window costs money. The dispatching team relied on experience and intuition to decide which trailers to move, where to position them, and in what order — but the yard was too large and the variables too numerous for any single person to track.

The actual requirement wasn't "automate the yard." It was: **give the dispatching team a ranked priority list of moves, so they could focus on the ones that mattered most instead of treating everything as equally urgent.**

That reframing — from full automation to decision support — shaped every technical choice that followed.

---

## Constraints

- **Noisy, incomplete data.** Historical records contained gaps, inconsistent formatting, and legacy fields whose meanings had drifted over time. Some features were derived from manual entry; others came from automated systems that didn't always agree.
- **Class imbalance.** The events we were trying to predict were, by definition, rare. The signal lived in the tail of the distribution, and standard accuracy metrics would have been misleading.
...
- **Interpretability mattered.** The dispatching team needed to understand why the model flagged something as high-priority. A black-box prediction they couldn't explain wouldn't be trusted, no matter how accurate it was.
- **Compute constraints.** The model had to run on available hardware — no cloud GPU instances, no managed ML services. Everything needed to fit on a local workstation.

---

## Phase 1: Data Preparation and Feature Engineering

The first challenge was making sense of the data. Historical records spanned multiple systems with different schemas, inconsistent naming conventions, and fields whose meanings had evolved over years of incremental changes. The data preparation phase was about creating a coherent feature set from this mess.

Missing values were handled through a combination of strategy selection and feature engineering — some gaps were filled with domain-informed defaults, others were converted into explicit "missing" indicator features so the model could learn whether absence of data was itself informative. Categorical variables were encoded using techniques appropriate for the tree-based models being used, avoiding the dimensionality explosion that one-hot encoding would have caused with high-cardinality fields.

Feature engineering drew heavily on domain knowledge. Temporal features were extracted from timestamps — time since last activity, day-of-week patterns, recency scores. Aggregate features captured historical behavior: how often a particular asset had been moved, the distribution of its past activity patterns, and how its behavior compared to similar assets. The goal wasn't to create the most features possible; it was to create the *right* features — ones that captured the operational logic the dispatching team already used, but in a form the model could learn from.
...
The baseline model used XGBoost's `XGBClassifier` with default settings, class-weight adjustment to handle the imbalanced dataset, and standard train-test splitting with stratification to preserve the class distribution. Evaluation went beyond accuracy — given the class imbalance, accuracy would have been meaningless. Instead, the focus was on F1-score, precision-recall curves, and decile-based gains analysis.
...
The baseline performance was... informative. It showed that the data contained a real signal — the model could do better than random — but there was significant room for improvement. The default parameters were clearly not optimized for this particular problem's characteristics: the class imbalance, the feature distributions, and the evaluation metric (F1-score) all suggested that the defaults were suboptimal.

This was the turning point that justified the investment in hyperparameter optimization.
...
**Phase 3a: Initial exploration (10 trials).** The first round was a broad sweep across the key hyperparameter spaces — `max_depth`, `min_child_weight`, `subsample`, `colsample_bytree`, `learning_rate`, and `reg_alpha`/`reg_lambda`. The goal wasn't to find the best parameters; it was to understand which dimensions of the search space mattered most.
...
The evaluation went beyond simple train-test metrics. Given the operational context — a ranked priority system for yard dispatching — the evaluation needed to answer practical questions: **Does the model actually help the dispatching team make better decisions?**

**Decile-based gains analysis.** The predictions were sorted by probability and divided into deciles. The actual event rate in each decile was calculated, producing a gains curve that showed how much of the signal the model captured at different cutoff points. A good model would show a steep drop-off: the top deciles should contain a disproportionately high concentration of events, while the bottom deciles should be mostly noise.

**Precision-recall analysis.** Given the class imbalance, the precision-recall curve was more informative than the ROC curve. It showed the tradeoff between precision (how many of the predicted events were actual events) and recall (how many of the actual events were captured) at different thresholds.
...
The optimized XGBoost model outperformed the default baseline across all metrics. The gains curve showed that the top 20% of predictions captured a disproportionate share of actual events — exactly the pattern you want in a priority system. The model's probability scores could be used to rank moves, and the feature importance analysis gave the dispatching team insight into *why* certain assets were flagged.

Measured conservatively, the system reduced the time the dispatching team spent reviewing low-priority moves by an estimated 40-50%. The honest version of the claim: the manual version of this problem would have required significantly more analyst time, and the quality of decisions would have been more variable depending on who was on shift.

---

## What I Learned

- **Bayesian optimization beats grid search.** The Optuna trials found better hyperparameters in far fewer evaluations than a grid search would have required. The Bayesian approach's ability to learn from previous trials and focus on promising regions of the search space was the key efficiency gain.
- **Stepwise optimization is practical.** Running the optimization in phases — initial exploration, then focused refinement — was more manageable than a single monolithic run. It allowed for course correction and kept the experiment tractable.
- **Resumable experiments matter.** Storing trials in SQLite and being able to pause and resume the optimization was essential for real-world workflow. It meant experiments could be interrupted without losing progress.
- **Decile analysis beats accuracy.** The gains curve and decile-based evaluation told a much more useful story than accuracy or even AUC. They showed *where* in the priority ranking the model was useful, which is the actual operational question.
...
- **The simplest deployment is often the right one.** A Python function that takes data in and predictions out, with no complex infrastructure, was exactly what the operational team needed. Complexity in deployment doesn't improve model performance.
- **Optuna's pruning saves compute.** The MedianPruner's ability to terminate unpromising trials early was a significant efficiency gain, especially for expensive models. It meant more trials could be run in the same compute budget.
