...
The training labels were the interesting engineering problem. No labeled dataset existed, so I mined the filename conventions into weak labels. These conventions reflected decades of humans encoding document types into filenames, complete with typos and drift. I wrote normalization rules for the misspelled variants and cross-referenced one category against a metadata database the review team maintained. Documents whose filenames were bare record numbers carried no usable signal; they became the unlabeled deployment set the models would later score. For each category I also trained two labeling variants: a broad one, where the type code appears anywhere in the filename, and a strict one, where the filename identifies exactly that code. I compared them to understand how label noise moved the results.
